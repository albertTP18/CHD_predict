'''
    Thuật toán Gradient Descent - thủ tục Backtracking
...
def GD_bt(w, x, y, alpha, beta, epsilon):
    i = 1  # Số vòng lặp
    loss_hist = [loss(w, x, y)]
    gradLoss_hist = [np.linalg.norm(grad_loss(w, x, y))]
    while True:
        t = 1
        gradLoss = grad_loss(w, x, y)
        norm_gradLoss = np.linalg.norm(gradLoss)
        while True:
            w_new = w - t * gradLoss

            if loss(w_new, x, y) - loss(w, x, y) <= - alpha * t * norm_gradLoss ** 2:
                break
            else:
                t = beta * t
...
w, loss_hist, gradLoss_hist = GD_bt(w, x_train, y_train, alpha, beta, epsilon)
